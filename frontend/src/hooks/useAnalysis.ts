...
export function useAnalysis() {
  ...
  const start = useCallback(async (studyId: string, ctx?: PatientContext, model?: string, features?: string[]) => {
    setJob(null); setSlices([]); setRawSlices([]); setReport(null)
    stopPolling()
    const newJob = await analysisApi.start(studyId, ctx, model, features)
    setJob(newJob)
