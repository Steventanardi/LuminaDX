...
const STATUS_LIGHT: Record<string, string> = {
  ...
  analyzing:  'bg-teal-50 text-teal-700 border border-teal-200',
}
const STATUS_DARK: Record<string, string> = {
  ...
  analyzing:  'bg-teal-950/60 text-teal-400 border border-teal-800/40',
}
...
export default function HistoryPanel({ open, onClose, isDark = false }: Props) {
  ...
  return (
    <>
      <div className={clsx('fixed inset-0 z-40 ', isDark ? 'bg-black/50' : 'bg-black/20')} onClick={onClose} />
      <aside className={clsx('fixed right-0 top-0 bottom-0 z-50 w-80 flex flex-col shadow-2xl border-l',
        isDark ? 'bg-[#0d1219] border-[#1f2835]' : 'bg-white border-[#e2e8ee]')}>
        <div className={clsx('flex items-center justify-between px-4 py-3 border-b shrink-0', isDark ? 'border-[#1f2835]' : 'border-[#e2e8ee]')}>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {loading ? (
            ...
            jobs.map(job => (
              <div key={job.job_id} className={clsx('rounded-xl border p-3 space-y-2', isDark ? 'bg-[#121924] border-[#1f2835]' : 'bg-white border-[#e2e8ee]')}>
